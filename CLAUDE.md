...
- Capo spellings live in `tests/lib/chordpro/capo-formats.test.ts`. Real sheets write it at least nine ways; add a case there before touching the regex. Prose matching is anchored to a line start or a bracket/separator, never to plain whitespace — that is what keeps "No capo" and lyrics using the word from matching — and prose is only read from the first 25 non-blank lines. UG also reports a capo as tab metadata, emitted as a `{capo:}` directive.
