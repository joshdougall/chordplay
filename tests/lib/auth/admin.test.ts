...
describe("requireAdmin", () => {
  ...
  it("returns null when no session", async () => {
    ...
    vi.doMock("@/lib/config", () => ({ getConfig: () => ({ adminUserIds: ["testadmin"] }) }));
  ...
  it("returns userId when user is in adminUserIds", async () => {
    vi.doMock("@/lib/auth/session", () => ({ getSession: async () => ({ userId: "testadmin" }) }));
    vi.doMock("@/lib/config", () => ({ getConfig: () => ({ adminUserIds: ["testadmin"] }) }));
    const { requireAdmin } = await import("@/lib/auth/admin");
    const result = await requireAdmin();
    expect(result).toEqual({ userId: "testadmin" });
  });

  it("supports multiple admins", async () => {
    vi.doMock("@/lib/auth/session", () => ({ getSession: async () => ({ userId: "secondadmin" }) }));
    vi.doMock("@/lib/config", () => ({ getConfig: () => ({ adminUserIds: ["testadmin", "secondadmin"] }) }));
    const { requireAdmin } = await import("@/lib/auth/admin");
    const result = await requireAdmin();
